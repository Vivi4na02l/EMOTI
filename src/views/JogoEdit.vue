    ...
    <div id="divEmotionsRoller" class="row">
      ...
      <div class="col-6 mx-5" id="divEditEmotion">
        ...
        <div v-if="selectedEmotion == ''">
          ...
          <picture>
            <source
              srcset="../assets/images/jogo/imagemDefault.webp"
              type="image/webp"
            />
            <source
              srcset="../assets/images/jogo/imagemDefault.png"
              type="image/png"
            />
            <img
              src="../assets/images/jogo/imagemDefault.png"
              class="mx-3 mt-5 mb-2 ol-3"
            />
          </picture>
        </div>
        ...
        <div v-else>
          <div id="divEmotionSelected">
            ...
            <div id="divImages">
              ...
              <button
                class="mx-3 mt-5 mb-2 col-3 btn"
                id="btnAddImage"
                @click="openDialogAddImage = true"
              >
                <h1>+</h1>
              </button>

              <img
                v-for="(image, index) in this.getSelectedImgs()"
                :key="index"
                :src="image.image"
                class="mx-3 mt-5 mb-2 col-3"
                style="background-color: transparent"
                data-toggle="tooltip"
                data-placement="top"
                title="Clique para eliminar a imagem!"
              />
            </div>
        ...
        <div class="mt-3 mb-5">
          <span id="spanForm">
            ...
            <div>
              <label for="sltEmotions" class="mr-2"
                >Selecione uma emoção para editar:</label
              >
      ...
      >
        <button
          type="button"
          class="close"
          @click="openDialogAddEmotion = false"
        >
          &times;</button
        ><br /><br />
        <form @submit.prevent="addEmotion()">
          <div>
            <label for="txtNewEmotion" class="mr-2"
              >Nome da nova emoção:
            </label>
            <input
              type="text"
              id="txtNewEmotion"
              v-model="form.txtNewEmotion"
            />
          </div>
          ...
          <div class="mt-5">
            ...
            <span v-for="(image, index) in newEmotionImages" :key="index">
              ...
              <br v-if="index >= 1" />
              <label v-if="index >= 1" style="visibility: hidden"
                >Link imagem:
              </label>
              ...
              <button
                type="button"
                @click="addNewImage(image.image)"
                :disabled="index + 1 != newEmotionImages.length"
                class="btn"
                id="btnPlus"
              >
                [ + ]
      ...
      <dialog
        ...
        style="width: 50%"
      >
        <button type="button" class="close" @click="openDialogAddImage = false">
          &times;</button
        ><br /><br />
        <form @submit.prevent="addImage()">
          <div class="mt-2">
            ...
            <span v-for="(image, index) in newImagesToEmotion" :key="index">
              ...
              <button
                type="button"
                @click="addNewImageToEmotion(image.image)"
                :disabled="index + 1 != newImagesToEmotion.length"
                class="btn"
                id="btnPlus"
              >
                [ + ]
  ...
  methods: {
    addNewImage(image) {
      if (image == "imagens/jogo/imagemDefault.png") {
        ...
        this.newEmotionImages.push({
          addedBy: "user",
          image: "imagens/jogo/imagemDefault.png",
        });
      }
    },
    ...
    getSelectedImgs() {
      if (
        this.arrayRecognizeEmotion.find(
          (pos) => pos.name == this.selectedEmotion
        )
      ) {
        return this.arrayRecognizeEmotion.find(
          (pos) => pos.name == this.selectedEmotion
        ).images;
      }
    },
    ...
    addImage() {
      this.$store.commit("MUTATE_EMOTION", {
        emotion: this.selectedEmotion,
        images: this.newImagesToEmotion,
      });
    },
  },
};
